function htmltext(text) {
    ...
    if (text == window.parent.GetTextByName("RealTimeTrace", window.parent.useprameters.languagedata)) {//多语言:实时轨迹
        returntext = window.parent.tracetext(window.parent.useprameters.rightselectid);
    } else if (text == window.parent.GetTextByName("LockFunction", window.parent.useprameters.languagedata)) {//多语言:开启锁定

    } else if (text == window.parent.GetTextByName("UnLockFunction", window.parent.useprameters.languagedata)) {//多语言:关闭锁定
        returntext = window.parent.locktext(window.parent.useprameters.rightselectid);
    } else {
        returntext = text;
    }
...
(function ($) {
    ...
    $.fn.smartMenu = function (data, options) {
        var B = $("body", window.parent.document), defaults = {
            name: "",
            offsetX: 2,
            offsetY: 2,
            textLimit: 12,
        ...
        var htmlCreateMenu = function (datum) {
            ...
            var idKey = "#smartMenu_", clKey = "smart_menu_", jqueryMenu = $(idKey + params.name, window.parent.document);
            if (!jqueryMenu.size()) {
                // $(params.obj).append(htmlCreateMenu());
                $("body", window.parent.document).append(htmlCreateMenu());

                //事件
                $(idKey + params.name + " a", window.parent.document).bind("click", function () {
                ...
                $(idKey + params.name + " li", window.parent.document).each(function () {
                ...
                return $(idKey + params.name, window.parent.document);
            }
            return jqueryMenu;
        };
        ...
        $(this).each(function () {
            this.onclick = function (e) {
               ...
               var position=window.parent.document.getElementById("select_user");
               var leftp=0
               if (position)
               {
               leftp =parseInt(position.style.left);
               }
