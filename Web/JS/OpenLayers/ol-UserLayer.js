...
function imgLoad(img, typename) {
    var timer = setInterval(function () {
        if (img.complete) {
            for (var i = 0; i < 16; i++) {
                var base64 = getBase64_UserType(img, i);
                //alert(base64);
                myMap.set(typename + "_" + i, base64);
...
function getBase64_UserType(source, index) {
            ...
            if (typeof (event.selected[0].values_.layer) != "undefined") {
                if (event.selected[0].values_.layer == "police") {
                    ...
                    jquerygetNewData_ajax("/Handlers/GetUserInfoByID_Handler.ashx", param, function (request) {
                        ...
                        if (bubbInfo.indexOf("Battery") != "-1") {//xzj--20181224--添加场强信息--手台电量
                            content.innerHTML += '<br/><code>' + battery + ': ' + (request[0].battery || "") + '</code>';
                        }
                        if (bubbInfo.indexOf("MsRssi") != "-1") {//--手台场强
                            content.innerHTML += '<br/><code>' + msRssi + ': ' + (request[0].msRssi || "") + '</code>';
                        }
                        if (bubbInfo.indexOf("UlRssi") != "-1") {//--上行场强
                            content.innerHTML += '<br/><code>' + ulRssi + ': ' + (request[0].ulRssi || "") + '</code>';
                        }
        ...
        function drawStart() {
            ...
            $("#map").mousemove(function (e) {
                ...
                if (ringvalue == "lengthss") {
                    ...
                    Lengthss = Math.round(lengths) + unit;
                } else if (ringvalue == "area") {
...
function ReLoadUser() {
        ...
        if (hide_timeout_device != "" && hide_timeout_device != useprameters.hide_timeout_device) {
            ...
            if (useprameters.hide_timeout_device) {
                ...
                for (var i = 0; i < Ids.length; i++) {
                    ...
                    var l = getLayerById(map, "realTime" + Ids[i]);
                    if (l)
                        map.removeLayer(l);
                }
    ...
    jquerygetNewData_ajax_post("WebGis/Service/LayerControl.aspx", param, function (request) {
        ...
        for (var i = 0; i < length; i++) {
            ...
            document.getElementById("preview").src = myMap.get(type + "_1");
            var imgsrc1,imgsrc2;
            ...
            if (featurePolice) {
                var lon1 = featurePolice.get("person").lon;
                var lat1 = featurePolice.get("person").lat;
                coordinate1 = ol.proj.transform([parseFloat(lon1), parseFloat(lat1)], 'EPSG:4326', 'EPSG:3857');
                positionIndex = featurePolice.get("person").direction;
            }


            positionIndex = getUserDirection(coordinate1, coordinate, positionIndex);
            
            var result = drawUserImages(type, internalSeconds, device_timeout, positionIndex);
            ...
            //-----------------------------end--------------------------------------------------------
            var Person = new Personinfo(info, entity, ISSI, type, lat, lon, gpsStatus, sendTime, terminalType, terminalStatus, ipAddress, mobile, zhiwu, bz, userId, trace, isRealTimeTrace, "new", positionIndex, msRssi, ulRssi, battery, result.isOnline);
            ...
            txtfeature.setStyle(
                ...
                if (featurePolice && (coordinate1[0] != coordinate[0] || coordinate1[1] != coordinate[1])) {
                    userMoveAnimate(userId, [featurePolice, featureTxt], [userfeature, txtfeature], result.imageArray, Person, coordinate1, coordinate);
                } else {
    ...
    var drawUserImages = function (policeType, internalSeconds, device_timeout, positionIndex) {
        ...
        var array = canvasMap.get(policeType + str + positionIndex);
        if (!array) {
            var imgArray = new Array();

            var img1 = myMap.get(policeType + str + positionIndex);
            var img2 = myMap.get(policeType + str + (positionIndex + 1));
            imgArray.push(img1);
            imgArray.push(img2);
            imgArray.forEach(function (img) {
                ...
                var image = new Image();
                image.src = img;
                context.drawImage(image, sx, sy, sWidth, sHeight, sx, dy, dWidth, dHeight);
                var scale = getCurrentScale(1);
                ...
                canvasMap.set(policeType + str + positionIndex, imageArray);
            });
        } else {
            imageArray = array;
        }
...
var addUserFeature = function (userFeature, txtFeature, person, userId) {
    ...
    if (isOpenheadInfo == "open") {
        ...
        txtFeature.getStyle().getText().setOffsetY(scale * (-64 / 2 - 20));
...
var userMoveAnimate = function (userId, oldfeatures, newfeatures, imageArray, person, from, to) {
    ...
    var userMoveItv = setInterval(function () {
        ...
        resetAllFlagPosition(userId, position, isResetOverlay);
        stepCount++;
    }, totalTime / stepNum);
}
...
var addUserArray = function (id) {
    ...
    if (feature) {
        ...
        userArray.push({
            ...
            "terminalStatus": person.terminalStatus, "ipAddress": person.ipAddress, "online": person.online
        });
    }
}
