...
function FieldStrengthLayer(thismap, opt_options) {
    ...
    var fieldStrengthSource = new ol.source.Vector();//场强图层数据源
    ...
    var currentUserLayer = getLayerById(currentMap, "user");//获取当前人员图层
    ...
    */
    ...
    //绘制图片位置设置，type表示类型，array表示需要展示的类型数组
    var ImagePosition_ = function (type, array) {
        var offsety = 2.2;//偏移Y
        var step = 0.5;//间距
    ...
    var newStyle = function (type, option) {
        switch(type){
            case FeatureType_.ENERGY:
                if (isShowEnergy) {
                    var style = returnStyle(drawEnergy, ImagePosition_, type, option.energy, [opt.ewidth, opt.eheight], option.online, FeatureType_.ENERGY);
                    return style;
                } else {
                    return null;
                }
            ...
            case FeatureType_.SIGNALINTENSITYUP:
                if (isShowUp) {
                    var style = returnStyle(drawSignalUp, ImagePosition_, type, option.signalup, [opt.swidth, opt.sheight], option.online, FeatureType_.SIGNALINTENSITYUP);
                    return style;
                } else {
                    return null;
                }
            ...
            case FeatureType_.SIGNALINTENSITYDOWN:
                if (isShowDown) {
                    var style = returnStyle(drawSignalDown, ImagePosition_, type, option.signaldown, [opt.swidth, opt.sheight], option.online,FeatureType_.SIGNALINTENSITYDOWN);
                    return style;
                } else {
                    return null;
                }
    ...
    //返回样式
    var returnStyle = function (drawFun, positionFun, type, value, imgSize, online, mark) {
        ...
        image.setAnchor(positionFun(type, showArray));
    ...
    var resetScale = function(features){
        var view = currentMap.getView();
                    ...
                    if (id.indexOf("p") == 0) {
                        ...
                        if (!isHaving) {
                            addFeature({
                                ...
                                energy: energy, signalup: signalup, signaldown: signaldown, online: online
                            });//如果该用户以前不存在，添加要素
                        }
                    }
