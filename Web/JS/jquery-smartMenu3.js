function htmltext(text) {
    ...
    if (text == window.parent.parent.parent.GetTextByName("LockFunction", window.parent.parent.parent.useprameters.languagedata)) {//多语言:开启锁定

    } else if (text == window.parent.parent.parent.GetTextByName("UnLockFunction", window.parent.parent.parent.useprameters.languagedata)) {//多语言:关闭锁定
        returntext = window.parent.parent.parent.locktext(window.parent.parent.parent.useprameters.rightselectid);
    } else {
        returntext = text;
    }
...
(function ($) {
    ...
    $.fn.smartMenu = function (data, options) {
        var B = $("body", window.parent.parent.parent.document), defaults = {
            name: "",
            offsetX: 2,
            offsetY: 2,
            textLimit: 12,
        ...
        var htmlCreateMenu = function (datum) {
            ...
            var idKey = "#smartMenu_", clKey = "smart_menu_", jqueryMenu = $(idKey + params.name, window.parent.parent.parent.document);
            if (!jqueryMenu.size()) {
                // $(params.obj).append(htmlCreateMenu());
                $("body", window.parent.parent.parent.document).append(htmlCreateMenu());

                //事件
                $(idKey + params.name + " a", window.parent.parent.parent.document).bind("click", function () {
                ...
                $(idKey + params.name + " li", window.parent.parent.parent.document).each(function () {
                ...
                return $(idKey + params.name, window.parent.parent.parent.document);
            }
            return jqueryMenu;
        };
            ...
            this.onclick = function (e) {
                ...
                if (jqueryMenu) {
                    jqueryMenu.css({
						...
						top: e.clientY  + params.offsetY - offsetvalue
                    });
