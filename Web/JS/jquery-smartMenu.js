function htmltext(text) {
    ...
    if (GetTextByName("RealTimeTrace", useprameters.languagedata).indexOf(temptext)>=0) {//多语言:实时轨迹
        returntext = tracetext(useprameters.rightselectid);
    } else if (text == GetTextByName("LockFunction", useprameters.languagedata)) {//多语言:开启锁定

    } else if (text == GetTextByName("UnLockFunction", useprameters.languagedata)) {//多语言:关闭锁定
        returntext = locktext(useprameters.rightselectid);
    } else {
        returntext = text;
    }

    //if (text == GetTextByName("RealTimeTrace", useprameters.languagedata)) {//多语言:实时轨迹
    //    returntext = tracetext(useprameters.rightselectid);
    //} else if (text == GetTextByName("LockFunction", useprameters.languagedata)) {//多语言:开启锁定
			...
			this.oncontextmenu = function(e) {
				...
				if (jqueryMenu) {
					jqueryMenu.css({
						...
						top: e.clientY + st + params.offsetY - offsetvalue
