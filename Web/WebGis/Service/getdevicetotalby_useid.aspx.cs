...
{
    public partial class getdevicetotalby_useid : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            DataTable dt = SQLHelper.ExecuteRead(CommandType.Text, "WITH lmenu(name,id) as (SELECT name,id  FROM [dbo].[Entity] WHERE id=@id UNION ALL    SELECT A.NAME,A.id FROM [dbo].[Entity] A,lmenu b    where a.ParentID = b.[ID])  SELECT COUNT(*) FROM [GIS_info] where USER_ID in (SELECT [id] FROM [User_info] where [Entity_ID] in (select id from lmenu)) and Send_reason not in ('Subscriber_unit_is_powered_OFF','DMO_ON') and DATEDIFF(MINUTE,Send_time,GETDATE()) < " + devicetimeout + " union all  SELECT COUNT([id]) FROM [User_info] where [Entity_ID] in (select id from lmenu)", "group", new SqlParameter("id", ID));
