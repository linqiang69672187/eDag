...
{
    /// <summary>
    /// GetGPSStatusList 的摘要说明
    /// </summary>
    public class GetGPSStatusList : IHttpHandler, IReadOnlySessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (!String.IsNullOrEmpty(selwhere)) {
                strWhere += " and (a.ISSI like '%" + stringfilter.Filter(selwhere.Trim()) + "%' or b.Nam like '%" + stringfilter.Filter(selwhere.Trim()) + "%' ) ";
            }
            if (!String.IsNullOrEmpty(begtime))
            {
                strWhere += " and OperatorTime >= '" + begtime + "'";
            }
            if (!String.IsNullOrEmpty(endtime))
            {
                strWhere += " and OperatorTime <= '" + endtime + "'";
            }
            ...
            sbSQL.Append(" SELECT b.id as id,a.[ISSI],[Runner],[OperatorTime],[GPSOpen],[OpenStatues],[GPSCircle],[CircleStatues],[GPSDestination],[DestinationStatues],[GPSorBEIDOU],[GPSorBEIDOUStatues],[GPSDistance],[GPSDistanceStatues],b.nam, c.ipAddress,ROW_NUMBER() over(order by [OperatorTime] desc) rownms");
            sbSQL.Append(" FROM GPSControlStatues a inner join User_info b on (a.issi=b.ISSI) left join ISSI_info c on (a.issi=c.ISSI) left join UserType d on (b.type = d.TypeName) ");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (dr["OpenStatues"].ToString() == "-1" || dr["OpenStatues"].ToString() != dr["GPSOpen"].ToString())
                {
                ...
                else if (dr["OpenStatues"].ToString() == dr["GPSOpen"].ToString() || dr["OpenStatues"].ToString() == "128" )//相等的时候
                {
            ...
            sb2.Append("select a.id  FROM GPSControlStatues a inner join User_info b on (a.issi=b.ISSI) left join ISSI_info c on (a.issi=c.ISSI) left join UserType d on (b.type = d.TypeName) where b.Entity_ID in (select  id from lmenu) ");
            foreach (var t in usertypes)
            {
                string idStr = string.Empty;
                foreach (var id in t.usertypeIds)
