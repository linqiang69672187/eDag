...
var varNow = new Date();
var begtime = varNow.getYear().toString() + varNow.getMonth().toString() + varNow.getDay().toString() + varNow.getHours().toString() + varNow.getMinutes().toString() + varNow.getSeconds().toString() + varNow.getMilliseconds().toString();
...
function REGMsg(msg, dispName) {
    ...
    if (msg == GetTextByName("REG_SUCCESS", useprameters.languagedata) || msg == GetTextByName("LINK_SUCCESS", useprameters.languagedata)) {
        //启用呼叫栏
        if (msg == GetTextByName("REG_SUCCESS", useprameters.languagedata)) {
            CallbannerEnable();
        }
        ...
        switch (Activetype.toString()) {
            ...
            case "2":
                if (msg.indexOf(GetTextByName("PATCH_CC_GRANTED", useprameters.languagedata)) >= 0 || msg.indexOf(GetTextByName("PATCH_CC_CALLREQUEST", useprameters.languagedata)) >= 0 || msg.indexOf(GetTextByName("PATCH_CC_CONNECT", useprameters.languagedata)) >= 0) {//多语言：获取授权；有呼叫请求进入;对方已接听
                    ...
                    if (obj.Iscall != null)//xzj--20181124--若由手台先发起呼叫，则在此将Iscall判断改为true防止调度台能未结束直接解派导致显示信息错乱
                    {
                        if (!obj.Iscall)
                        {
                            obj.Iscall = true;
            ...
            case "00": //表示单呼半双工
                ...
                if (isCase1 || isCase2) {
                    ...
                    if (msg == GetTextByName("PPC_RELEASE", useprameters.languagedata)) {//开始按钮显示（呼叫），结束按钮隐藏，用户名可改，用户选择按钮可见，呼叫类型可改 多语言：对方挂断
                        document.frames["PPCCall_ifr"].document.getElementById("begincall").style.display = "";
                        ...
                        endppcSC(issi);
                        document.frames["PPCCall_ifr"].document.getElementById("imgSelectUser").style.display = "";
                        ...
                        if (isCase1) {//变成接听的界面，还需要处理自动接听的 
                            if (hookmethodsel == "0") {//自动应答模式 
                                ...
                                calltype[2].disabled = false;
                                document.frames["PPCCall_ifr"].document.getElementById("trUserBtn").style.display = "";
                                document.frames["PPCCall_ifr"].document.getElementById("begincall").style.display = "";
                                document.frames["PPCCall_ifr"].document.getElementById("endcall").style.display = "";
                                document.frames["PPCCall_ifr"].document.getElementById("begincall").innerHTML = "PTT";
...
//MSG: 基站呼叫
function BCMsg(switchID,issi, bctype, msg) {//xzj--20190304--添加交换ID
    //将消息翻译成相关语言 逻辑中就使用相关语言
    msg = GetTextByName(msg, useprameters.languagedata);//多语言版才能使用

    mapMsgShow(switchID,issi, bctype, msg);
    ...
    if (document.frames["SBCall_ifr"] != null || document.frames["SBCall_ifr"] != undefined) {
        ...
        switch (bctype.toString()) {
            case "3":
                ...
                if (msg.indexOf(GetTextByName("BC_RELEASE", useprameters.languagedata)) >= 0) {//多语言：对方已挂断
                    ...
                    document.frames["SBCall_ifr"].document.getElementById("txtSwitch").disabled = false;//xzj--20190318--设置交换不能输入
        ...
        if (msg.indexOf(GetTextByName("BC_RELEASE", useprameters.languagedata)) < 0) {//xzj--20190227--产生两次回调，第二次回调信息为对方已挂断，不符合操作，使用第一次的回调信息结束单站广播
...
function DLMsg(issi, report, failreason, altype) {
    ...
    if (document.frames["DLCall_ifr"] != null || document.frames["DLCall_ifr"] != undefined) {
        ...
        if ((altype == 0) && (report == GetTextByName("DL_START_SUCCEED", useprameters.languagedata))) {//强插强插可见,结束按钮可见 多语言：开始成功
            ...
            document.frames["DLCall_ifr"].document.getElementById("trBegJT").style.display = "table-row";
            document.frames["DLCall_ifr"].document.getElementById("trQCQC").style.display = "table-row";
            ...
            if (document.frames["PrivateCall_ifr"].document.getElementById("txtISSIText") != null && issi == document.frames["PrivateCall_ifr"].document.getElementById("txtISSIText").value.trim()) {
                ...
                if (eventtype == "00") {//表示单呼半双工
                    ...
                    if (msg == GetTextByName("CC_RELEASE", useprameters.languagedata)) {//多语言：对方挂断
                        ...
                        document.frames["PrivateCall_ifr"].document.getElementById("isEncryption").disabled = false;//xzj--20190320--添加呼叫加密参数
                        iszd = true;
                        endSC2(issi);
                    }
            ...
            if (document.frames["PrivateCall_ifr"].document.getElementById("txtISSIText") != null) {
                ...
                if (issi == document.frames["PrivateCall_ifr"].document.getElementById("txtISSIText").value.trim()) {
                    ...
                    if (msg == GetTextByName("CC_CONNECTACK", useprameters.languagedata)) {//多语言：呼叫已连接
                        ...
                        document.frames["PrivateCall_ifr"].document.getElementById("isEncryption").disabled = true;//xzj--20190320--添加呼叫加密参数
                    }
...
//MSG: 短信消息
function SMSMsg(issi, smstype, msg, id, strisconsume,switchID) {//xzj--20190305--添加交换
    //过滤逗号
    ...
    if (smstype == "01" || smstype == "31") { //首台发送消息为空时
    ...
    else if (smstype == "0" || smstype == "02" || smstype == "3") { //表示收到的普通消息内容；表示收到的状态消息 不能进行多语言转化；02表示调度台收到来自调度台发来的短信；
        var MYName = "";
        var Lo = "";
        var La = "";
        var PCId = "";
        ...
        todbMSG = GetTextByName(msg, useprameters.languagedata);
        var param = { "issi": issi };
        if (msg == "SM_RECEIPT_ACK_BY_DESTINATION")//xzj--20190225--添加基站短信
        {
            param = { "issi": issi, "switchID": switchID };
        }
            ...
            if (msg != "GENERAL_STATUS_ACK") {
                switch (_data.stype) {
                    ...
                    case "U":
                        ...
                        break;
                        case "B"://xzj--20190225--添加基站短信--20190311--基站名称参数添加单引号
                        var myMsg = "[" + GetTextByName("Send(SMS)", useprameters.languagedata) + "]" + GetTextByName(msg, useprameters.languagedata) + "(<span   oncontextmenu=javascript:rightselecttype='baseStation';rightBSID=" + _data.bsid + ";rightBSISSI=" + _data.pcid + ";rightBSName='" + _data.sname +"';rigthSwitchID="+_data.switchID+ "   onclick=window.parent.parent.mycallfunction('Send_SMS','380',400,'" + issi + "&cmd=BSISSI&name=" + encodeURI(MYName) + "&switchID=" + _data.switchID + "') >" + MYName + ":" + issi + "(" + _data.switchID + ")" + "</span>)[" + LOGTimeGet() + "]";//多语言：发送(短信)
                        writeLog("sms", myMsg);
                        break;
                    default:
...
function doRegX() {
    ...
    var scactionX = document.frames['log_windows_ifr'].document.getElementById('SCactionX');
    if (scactionX == null) {
        setTimeout(doRegX, 1000);
        return;
    }

    console.log(scactionX.classid)
    // var url = kdocTitle;
...
function regX() {
    if (isLoadLog == 'log' && isLoadMain == 'main' && isLoadSys == 'SYS') {
        ...
        if (varNow.getYear().toString() + varNow.getMonth().toString() + varNow.getDay().toString() + varNow.getHours().toString() + varNow.getMinutes().toString() + varNow.getSeconds().toString() + varNow.getMilliseconds().toString() - begtime < 15000) {
...
//添加基站组成员
function AddMBSGroupMem(BsgID, switchID, MemberID) {//xzj--20181217--添加交换
    try {
        var scactionX = document.frames['log_windows_ifr'].document.getElementById('SCactionX');
        return scactionX.AddorDelMBSGroupMem(BsgID, switchID, MemberID, 0);
    } catch (ex) {
        writeLog("system", ex);
        return false;
    }
...
function startDC() {
    ...
    try {
        if (useprameters.SelectISSI && useprameters.SelectISSI.length == 1) {
            ...
            var scactionX = document.frames['log_windows_ifr'].document.getElementById('SCactionX');
            var reval = scactionX.StartDCall(useprameters.SelectISSI[0], isEncryption);
...
//控制面板里面使用
//开始单基站呼叫
function startSBCall(switchID, phone) {//xzj--20181217--添加交换
    var content = "";
    try {
        var scactionX = document.frames['log_windows_ifr'].document.getElementById('SCactionX');
        var reval = scactionX.StartSBCall(switchID,phone);
        writeLog("oper", "[" + GetTextByName("Lang_SingleStationCall", useprameters.languagedata) + "]:" + GetTextByName("Lang_DispatchBegBaseStationCall", useprameters.languagedata) + "(SwitchID:" + switchID + ")" + "(ISSI:" + phone + ")" + "[" + LOGTimeGet() + "]");         /**日志：操作日志 多语言：单站呼叫;调度台操作窗口开始基站呼叫**/
        WriteBSLo(GetTextByName("Lang_BegSingleBaseStationCall", useprameters.languagedata) + "("+switchID+"," + phone + ")");// 多语言：开始单站基站呼叫
        //写入数据库日志
...
//结束单基站呼叫
function endSBCall(switchID, phone) {//xzj--20181217--添加交换
    var content = "";
    try {
        var scactionX = document.frames['log_windows_ifr'].document.getElementById('SCactionX');
        var reval = scactionX.EndSBCall(switchID,phone);
        writeLog("oper", "[" + GetTextByName("Lang_SingleStationCall", useprameters.languagedata) + "]:" + GetTextByName("Lang_DispatchEndBaseStationCall", useprameters.languagedata) + "(SwitchID:" + switchID + ")" + "(ISSI:" + phone + ")" + "[" + LOGTimeGet() + "]");         /**日志：操作日志 多语言：单站呼叫;调度台操作窗口结束基站呼叫**/
        WriteBSLo(GetTextByName("Lang_EndSingleBaseStationCall", useprameters.languagedata) + "(" + switchID + "," + phone + ")");//多语言：单站呼叫；结束单站基站呼叫
        //写入数据库日志
...
function startDC2(phone, isEncryption) {//xzj--20190320--添加呼叫加密参数
    var content = "";
    try {
        //写入数据库日志
        content = "Log_BegFullSigle_Success";
        var scactionX = document.frames['log_windows_ifr'].document.getElementById('SCactionX');
        var reval = scactionX.StartDCall(phone, isEncryption);
...
function startSC2(phone, isEncryption) {//xzj--20190320--添加呼叫加密参数
    var content = "";
    try {
        //写入数据库日志
        content = "Log_BegHalfSigle_Success";
        var scactionX = document.frames['log_windows_ifr'].document.getElementById('SCactionX');
        var reval = scactionX.StartSCall(phone, isEncryption);
...
function startGC2(phone, isEncryption) {//xzj--20190320--添加呼叫加密参数
    var content = "";
    try {
        content = "Log_BegGroupCall_Success";
        var scactionX = document.frames['log_windows_ifr'].document.getElementById('SCactionX');
        var reval = scactionX.StartGCall(phone, isEncryption);
...
function gceasedPTT2(phone) {
    try {
        ...
        writeLog("oper", "[" + GetTextByName("SingleCALL(half)", useprameters.languagedata) + "]:" + GetTextByName("Lang_DispatchCallPanelGceasedPTT", useprameters.languagedata) + "(GSSI:" + phone + ")" + "[" + LOGTimeGet() + "]");         /**日志：操作日志 多语言：单呼(半);调度台操作窗口释放授权**/
        return reval;
    } catch (ex) {
        writeLog("system", ex);
        return false;
...
function sceasedPTT2(phone) {
    try {
        ...
        writeLog("oper", "[" + GetTextByName("SingleCALL(half)", useprameters.languagedata) + "]:" + GetTextByName("Lang_DispatchCallPanelSceasedPTT", useprameters.languagedata) + "(GSSI:" + phone + ")" + "[" + LOGTimeGet() + "]");         /**日志：操作日志 多语言：单呼(半);调度台操作窗口获取授权**/
        //var map = document.getElementById("main");
        //map.Cancelringing();//调用FLASH呼叫栏
        return reval;
    } catch (ex) {
...
function startSC() {
    ...
    try {
        if (useprameters.SelectISSI && useprameters.SelectISSI.length == 1) {
            ...
            var scactionX = document.frames['log_windows_ifr'].document.getElementById('SCactionX');
            scactionX.StartSCall(useprameters.SelectISSI[0], isEncryption);
            ...
            if (mi < users.length) {
                if (mi < LTEUsers.length) {
                    ...
                    window.parent.jquerygetNewData_ajax_post("Handlers/AddGPSParam.ashx", { mtype: "add", cmd: "control", open: isEnable, sendGPSParamRequestissis: sendGPSParamRequestissis }, function (msg) {
                        //执行SearchGpsStatues页面的getData（）函数，重新加载列表数据-----------------xzj--2018/7/12-------------------------
                        var iframe = window.parent.document.getElementById("SearchGpsStatues_ifr");
                        iframe.contentWindow.getData();
                        //-----------------------------------------------------------
                    });
            ...
            if (i < users.length) {
                if (users[i].issitype.trim() == "LTE") {
                    if (hasDoIssiOpen != users[i].uissi) {
                        try {
                            var retVar = scactionX.SendModifyGpsInterval(1, users[i].uissi, times == "" ? 0 : times, isEnable, 0, "");//xzj--20190114--times参数用三元表达式为空则传0
                            if (retVar == 1) {
...
function GPSMsg(ISSI, resultCode, ctypeResponse) {
    ...
    if (ctypeResponse == "GPS_LOCATION_REPORTING_ENABLE_DISABLE_RESPONSE")  //GPS使能开口
    {
        ...
        issi_pullup_isEnable = "1";
        if (!isPullUp)
        { return; }
        for (var i = 0; i < usersInfo.length; i++) {
            if (usersInfo[i].uissi.toString().trim() == ISSI.toString().trim()) {
                statues = strFailed;
                //修改上拉计数不断累加的问题，修改人：张谦
                //---start-----
                if (sbPullUp.indexOf(ISSI + "," + strPullUpFail) > -1) {
                    var sbArrIndex = sbPullUp.indexOf(ISSI + "," + strPullUpFail);
                    sbPullUp.splice(sbArrIndex, 1);
                }

                if (sbPullUp.indexOf(ISSI + GetTextByName("PullUp_dispatch_ispullup", useprameters.languagedata) + ";") > -1) {
                    var sbArrIndex = sbPullUp.indexOf(ISSI + GetTextByName("PullUp_dispatch_ispullup", useprameters.languagedata) + ";");
                    sbPullUp.splice(sbArrIndex, 1);

                }

                if (sbPullUp.indexOf(ISSI + GetTextByName("PullUp_sendfail", useprameters.languagedata) + ";") > -1) {
                    var sbArrIndex = sbPullUp.indexOf(ISSI + GetTextByName("PullUp_sendfail", useprameters.languagedata) + ";");
                    sbPullUp.splice(sbArrIndex, 1);
                }

                if (sbPullUp.indexOf(ISSI + GetTextByName("PullUp_error_sendfail", useprameters.languagedata)) > -1) {
                    var sbArrIndex = sbPullUp.indexOf(ISSI + GetTextByName("PullUp_error_sendfail", useprameters.languagedata));
                    sbPullUp.splice(sbArrIndex, 1);
                }

                if (sbPullUp.indexOf(ISSI + "," + strPullUpOutTime) > -1) {
                    var sbArrIndex = sbPullUp.indexOf(ISSI + "," + strPullUpOutTime);
                    sbPullUp.splice(sbArrIndex, 1);
                }

                if (cgPullUp.indexOf(ISSI + "," + strPullUpSuccess) > -1) {
                    var cgArrIndex = cgPullUp.indexOf(ISSI + "," + strPullUpSuccess);
                    cgPullUp.splice(cgArrIndex, 1);
                }
                //----end---

                sbPullUp.push(ISSI + "," + strPullUpFail);
        ...
        for (var i = 0; i < usersInfo.length; i++) {
            if (usersInfo[i].uissi.toString().trim() == ISSI.toString().trim()) {
                ...
                //修改上拉计数不断累加的问题，修改人：张谦
                //---start-----
                if (cgWGPullUp.indexOf(ISSI + "," + strPullUpWGSuccess) > -1) {
                    var cgWGArrIndex = cgWGPullUp.indexOf(ISSI + "," + strPullUpWGSuccess);
                    cgWGPullUp.splice(cgWGArrIndex, 1);
                }
                //----end---
                cgWGPullUp.push(ISSI + "," + strPullUpWGSuccess);
                writeLog("system", strPullUpWGSuccess + ":" + ISSI + "(" + statues + ")");//写日志
                toWritePullUpResult(allShowedDoPullUpCount, hadDoPullUpcount);
                if (usersInfo.length == 1) cgUnPullUp.splice(0, cgUnPullUp.length);  //这里清空数组是为了单个上拉时不会重复显示一条记录的网管成功响应。
                return;
            }
        }
    } else if (ctypeResponse == "SM_PDT_PULLACTIVE_TE_SUCCESS") {                     //激活成功，终端响应成功--2
        issi_pullup_isEnable = "1";
        if (!isPullUp)
        { return; }
        for (var i = 0; i < usersInfo.length; i++) {
            if (usersInfo[i].uissi.toString().trim() == ISSI.toString().trim()) {
                ...
                cgPullUp.push(ISSI + "," + strPullUpSuccess);
                writeLog("system", strPullUpSuccess + ":" + ISSI + "(" + statues + ")");//写日志
                toWritePullUpResult(allShowedDoPullUpCount, hadDoPullUpcount);
                return;
            }
        ...
        for (var i = 0; i < usersInfo.length; i++) {
            if (usersInfo[i].uissi.toString().trim() == ISSI.toString().trim()) {
                ...
                sbPullUp.push(ISSI + "," + strPullUpOutTime);
                writeLog("system", strPullUpOutTime + ":" + ISSI + "(" + statues + ")");//写日志
                toWritePullUpResult(allShowedDoPullUpCount, hadDoPullUpcount);
                return;
            }
        ...
        if (!isPullUp)
        { issi_pullup_isEnable = "2"; }//上拉的时候详情页不关闭，自动去上拉后区分打印
        issi_pullup_isEnable = "0";
        for (var i = 0; i < usersInfo.length; i++) {
            if (usersInfo[i].uissi.toString().trim() == ISSI.toString().trim()) {
                statues = strFailed;
                ...
                //修改上拉计数不断累加的问题，修改人：张谦
                //---start-----
                if (sbUnPullUp.indexOf(ISSI + GetTextByName("PullUp_terminal_unpullup", useprameters.languagedata) + ";") > -1) {
                    var sbUnArrIndex = sbUnPullUp.indexOf(ISSI + GetTextByName("PullUp_terminal_unpullup", useprameters.languagedata) + ";");
                    sbUnPullUp.splice(sbUnArrIndex, 1);
                }
                if (sbUnPullUp.indexOf(ISSI + GetTextByName("PullUp_notfind_terminal", useprameters.languagedata) + ";") > -1) {
                    var sbUnArrIndex = sbUnPullUp.indexOf(ISSI + GetTextByName("PullUp_notfind_terminal", useprameters.languagedata) + ";");
                    sbUnPullUp.splice(sbUnArrIndex, 1);
                }
                if (sbUnPullUp.indexOf(ISSI + "," + strUnPullUpOutTime) > -1) {
                    var sbUnArrIndex = sbUnPullUp.indexOf(ISSI + "," + strUnPullUpOutTime);
                    sbUnPullUp.splice(sbUnArrIndex, 1);
                }

                if (sbUnPullUp.indexOf(ISSI + "," + strUnPullUpFail) > -1) {
                    var sbUnArrIndex = sbUnPullUp.indexOf(ISSI + "," + strUnPullUpFail);
                    sbUnPullUp.splice(sbUnArrIndex, 1);
                }

                if (cgUnPullUp.indexOf(ISSI + "," + strUnPullUpSuccess) > -1) {
                    var cgUnArrIndex = cgUnPullUp.indexOf(ISSI + "," + strUnPullUpSuccess);
                    cgUnPullUp.splice(cgUnArrIndex, 1);
                }
                //----end---

                sbUnPullUp.push(ISSI + "," + strUnPullUpOutTime);
                writeLog("system", strUnPullUpOutTime + ":" + ISSI + "(" + statues + ")");//写日志
                toWritePullUpResult(allShowedDoPullUpCount, hadDoPullUpcount);
                return;
            }
        }
        ...
        if (!isPullUp)
        { issi_pullup_isEnable = "2"; }
        issi_pullup_isEnable = "0";
        for (var i = 0; i < usersInfo.length; i++) {
            if (usersInfo[i].uissi.toString().trim() == ISSI.toString().trim()) {   //在上拉列表中找传过来的返回信息
                statues = strSuccess;
                ...
                cgUnPullUp.push(ISSI + "," + strUnPullUpSuccess);
                writeLog("system", strUnPullUpSuccess + ":" + ISSI + "(" + statues + ")");//写日志
                toWritePullUpResult(allShowedDoPullUpCount, hadDoPullUpcount);  //当详情页关闭后这里再继续发的话，网站会报js错误，因为获取不到详情页的对象。
                return;
            }
        }
        ...
        for (var i = 0; i < usersInfo.length; i++) {
            if (usersInfo[i].uissi.toString().trim() == ISSI.toString().trim()) {
                ...
                sbUnPullUp.push(ISSI + "," + strUnPullUpFail);
                writeLog("system", strUnPullUpFail + ":" + ISSI + "(" + statues + ")");//写日志
                toWritePullUpResult(allShowedDoPullUpCount, hadDoPullUpcount);
                return;
            }
...
function sendMsg(ISSIs, message, isConsume) {
    ...
    else if (ISSIs.length == 1 && ISSIs[0].type == GetTextByName("Station", useprameters.languagedata)) {//xzj--20190218--添加基站短信
        sendBaseStationMsg(ISSIs[0].issi, message, isConsume);
        return vreturn;
    }
    ...
    if (issiLenght > 0) {
        ...
        var sendInterval = setInterval(function () {
            if (issiindex < issiLenght) {
                ...
                else if (ISSIs[issiindex].type == GetTextByName("Station", useprameters.languagedata)) {//xzj--20190218--添加基站短信
                    sendBaseStationMsg(ISSIs[issiindex].issi, message, isConsume);
                }
                else {
...
function doSendMsg(issi, message, isConsume) {
    ...
    if (reVal == sendfailed) { //(reVal == "发送失败")
        ...
        jquerygetNewData_ajax_post("Handlers/AddSMS.ashx", { issi: issi, content: message, isreturn: isConsume, smsid: reVal, isgroup: 0, smsType: smsType_Commom_Msg }, function (msg) { });
    }
    return vreturn;
}
//xzj--20190218--添加基站短信
function sendBaseStationMsg(issi, message, isConsume) {
    var bsISSISwitchArray = issi.split(/[()]/);
    var vreturn = true;
    var scactionX = document.frames['log_windows_ifr'].document.getElementById('SCactionX');
    var reVal = 0;
    var content = "";
    var todbMSG= GetTextByName(constMsgSendSucess, useprameters.languagedata);
    try {
        var switchID = bsISSISwitchArray[1];
        var bsISSI = bsISSISwitchArray[0];
        reVal = scactionX.SendMSGToBS(message, switchID, bsISSI, isConsume);
        if (reVal == 0) {
            content = "Log_GPS_SMSSend_Failed;Log_GPS_SMS_Content:" + message + ";Log_GPS_SMS_IsHZ:" + isConsume;
        } else {
            content = "Log_GPS_SMSSend_Success;Log_GPS_SMS_Content:" + message + ";Log_GPS_SMS_IsHZ:" + isConsume;
        ...
        writeToDbByUserISSI(issi, OperateLogType.operlog, OperateLogModule.ModuleSMS, OperateLogOperType.CommSMSSend, content, OperateLogIdentityDeviceType.BaseStation);

    }
    var sendfailed = GetTextByName("SM_DELIVERY_FAILED", useprameters.languagedata)
    if (reVal == 0) { //(reVal == "发送失败")
        vreturn = false;
    }
    else {//xzj--20190227--添加基站短信
        jquerygetNewData_ajax_post("Handlers/AddSMS.ashx", { issi: issi, content: message, isreturn: isConsume, smsid: reVal, isgroup: 0,status:todbMSG, smsType: smsType_Commom_Msg }, function (msg) { });
    }
    return vreturn;
}
...
function sendreportnum(ISSIs, statusNum) {
    ...
    if (ISSIs.length == 1) { //当只有一个对象的时候特殊处理，不然循环发送的时候会出问题
        doSendStatusnum(ISSIs[0].issi, statusNum, ISSIs[0].type);
        return vreturn;
    ...
    if (issiLenght > 0) {
        ...
        var sendInterval = setInterval(function () {
            if (issiindex < issiLenght) {
                doSendStatusnum(ISSIs[issiindex].issi, statusNum, ISSIs[0].type);
                issiindex++;
            } else {
                clearInterval(sendInterval);
            }
...
function doSendStatusnum(issi, statusnum, type) {
    var isGroup = 0;
    var todbMSG = "";
    if (type == GetTextByName("Group", useprameters.languagedata))
        isGroup = 1;
    var scactionX = document.frames['log_windows_ifr'].document.getElementById('SCactionX');
    var content = "";
    try {
        ...
        jquerygetNewData_ajax("Handlers/AddSMS.ashx", { issi: issi, content: "STATUS:" + statusnum, status: todbMSG, isreturn: 0, smsid: -1, isgroup: isGroup, smsType: smsType_Status_Msg }, function (msg) { });
    }
}
...
function doSendStatusMsg(issi, message) {
    ...
    try {
        ...
        if (reVal == "12") {
            ...
            var num = "numb" + reVal;//弹出错误信息
            var jump = window.parent.parent.GetTextByName(num, window.parent.parent.useprameters.languagedata);
            alert(jump);
...
//被基站呼叫里面的消息调用
function mapMsgShow(switchID,issi, bctype, msg) {//msg不需要传进来了 在调用的接口以及多语言转化过了--xzj--20190304--添加交换ID
    ...
    var mymsg = msg;
    if (bctype == 3) {//xzj--20190305--只有单站呼叫执行以下
        jquerygetNewData_ajax("Handlers/GetBaseStationInfoByISSI.ashx", { issi: issi, switchID: switchID }, function (msgs) {//xzj--20190213--添加交换ID
            var bsi = eval(msgs);
            if (bsi.length > 0 && mymsg != GetTextByName("BC_RELEASE", useprameters.languagedata)) {//xzj--20190213--BC_RELEASE为“对方已挂断”不对，基站为广播
                document.getElementById("scrollarea").innerHTML = GetTextByName("StationName", useprameters.languagedata) + ":" + bsi[0].StationName + "(" + issi + "(" + switchID + ")" + ");" + atype + ";" + mymsg;//多语言：基站名称--xzj--20190305--添加交换ID

            }
        });
    }
    else {
        if (mymsg != GetTextByName("BC_RELEASE", useprameters.languagedata))
            {
            document.getElementById("scrollarea").innerHTML = atype + ";" + mymsg;//多语言：基站名称--xzj--20190305
        }
    }
}
function showradio() {
...
function issilocation(lo, la, ci, ISSI) {
    try {
        ...
        if (lo != 0 && la != 0) {
            ...
            useprameters.localid = pars.split('|')[1];
            var param = { "id": ci };
            jquerygetNewData_ajax("WebGis/Service/getlola_byID.aspx", param, function (request) {
                ...
                if (_data) {
                    ...
                    LocatePerUser(ci, ISSI, lo, la, isOnline);
                }
            });
...
function cameraLoation(cameraNum, lo, la) {
    ...
    try {
        if (lo != 0 && la != 0) {
            var mainSWF = document.getElementById("main");
            if (mainSWF) {
                if (mainSWF.getVideoSelect()) {
                    isSuccess = mainSWF.callbackLocateCamera(cameraNum, parseFloat(lo), parseFloat(la));
                    isSuccess = true;
                }
                else {
                    alert(GetTextByName("Lang_VideoLayerAlert", useprameters.languagedata));
                    isSuccess = false;
                }
            }
        }
        else {
            alert(GetTextByName("Lang_VideoLoLaWrong", useprameters.languagedata));
            isSuccess = false;
        }
...
var lastISSI = 0;
var lasttootl = 0;
var gssiDT = 0//2018.7.17虞晨超为 动态重组后弹出组呼 获取组呼id
function clear_p_run() {
    ...
    if (document.frames["DTGroup/ResultList_ifr"] != null || document.frames["DTGroup/ResultList_ifr"] != undefined) {
        ...
        window.parent.parent.mycallfunction('GroupCall', 380, 500, '&type=GSSI&myid=' + gssiDT, 1999);//2018.7.17虞晨超为 动态重组后弹出组呼 获取组呼id
    }
}
...
function SendAddMemberToDTGroup2(GSSI, users) {
    ...
    cjarry.length = 0;
    sbarry.length = 0;
    gssiDT = GSSI//2018.7.17虞晨超为 动态重组后弹出组呼 获取组呼id
    var mi = 0;
    if (mi < users.length) {
        var laset = users.length;
        if (parseFloat(mi) + 500 < parseFloat(laset)) {
...
function openPullUpContralResultPage() {
    if (document.frames["PullUpControl/PullUpResultList_ifr"] != null || document.frames["PullUpControl/PullUpResultList_ifr"] != undefined) {
        ...
        mycallfunction('PullUpControl/PullUpResultList', 830, 350);
    }
}
...
function toWritePullUpResult(totalcount, asdfasd) {
    ...
    if (document.frames["PullUpControl/PullUpResultList_ifr"]) {
        document.frames["PullUpControl/PullUpResultList_ifr"].toHTML(totalcount, hadDoPullUpcount, issi_pullup_isEnable, cgPullUp, sbPullUp, cgWGPullUp, cgUnPullUp, sbUnPullUp);
        timeN = 0;
    } else {
...
var cgWGPullUp = new Array();
var cgUnPullUp = new Array();
var sbUnPullUp = new Array();


//-----------------------GPS上拉新版本代码开始-------------------------
//isEnable  1表示开启，0表示关闭，新版本代码
        ...
        sendpullupcontrolInterval = setInterval(function () {
            if (i < userscount) {
                ...
                try {
                    ...
                    //alert(isEnable);
                    if (isEnable == '1') {
                        window.parent.jquerygetNewData_ajax_pullup("../../Handlers/AddGPSPullParam.ashx", { mtype: "add", srcissi: users[i].uissi, operatetype: "1", expire: totleCycle, period: times, distance: distance, dstissi: dest, gisissi: getCookieByName("dispatchissi"), result: "100", pullstatus: "0" }, function (msg) {
                            Pullstatus = msg;
                            //执行SearchGpsPullStatues页面的getData（）函数，重新加载列表数据-----------------xzj--2018/7/12-------------------------
                            var iframe = window.parent.document.getElementById("SearchGpsPullStatues_ifr");
                            iframe.contentWindow.getData();
                            //-----------------------------------------------------------
                        });
                    } else if (isEnable == '0') {
                        window.parent.jquerygetNewData_ajax_pullup("../../Handlers/AddGPSPullParam.ashx", { mtype: "update", srcissi: users[i].uissi, operatetype: "1", expire: totleCycle, period: times, distance: distance, dstissi: dest, gisissi: getCookieByName("dispatchissi"), result: "100", pullstatus: "0" }, function (msg) {
                            Pullstatus = msg;
                            //执行SearchGpsPullStatues页面的getData（）函数，重新加载列表数据-----------------xzj--2018/7/12-------------------------
                            var iframe = window.parent.document.getElementById("SearchGpsPullStatues_ifr");
                            iframe.contentWindow.getData();
                    ...
                    if (Pullstatus == '1') {
                        sbPullUp.push(users[i].uissi + GetTextByName("PullUp_dispatch_ispullup", useprameters.languagedata) + ";");
                        writeLog("system", users[i].uissi + GetTextByName("PullUp_dispatch_ispullup", useprameters.languagedata));
                    } else if (Pullstatus == 0) {
                        if (dest.trim() == "") {
                            retVar = scactionX.SendPdtPullGpsRequest_New(isEnable, 1, users[i].uissi, 0, getCookieByName("dispatchissi"), times, totleCycle, distance);
                        } else {
                            retVar = scactionX.SendPdtPullGpsRequest_New(isEnable, 1, users[i].uissi, dest, getCookieByName("dispatchissi"), times, totleCycle, distance);
                        }
                    } else if (Pullstatus == '2') {
                        retVar = scactionX.SendPdtPullGpsRequest_New(isEnable, 1, users[i].uissi, dest, getCookieByName("dispatchissi"), times, totleCycle, distance);
                    } else if (Pullstatus == '3') {
                        sbUnPullUp.push(users[i].uissi + GetTextByName("PullUp_terminal_unpullup", useprameters.languagedata) + ";");
                        writeLog("system", users[i].uissi + GetTextByName("PullUp_terminal_unpullup", useprameters.languagedata));
                    }
                    else if (Pullstatus == '4' || Pullstatus == undefined) {
                        sbUnPullUp.push(users[i].uissi + GetTextByName("PullUp_notfind_terminal", useprameters.languagedata) + ";");
                        writeLog("system", users[i].uissi + GetTextByName("PullUp_notfind_terminal", useprameters.languagedata));
                    }

                    if (retVar == 0 && Pullstatus != 1) {   //retVar返回1表示成功下发,retVar返回0表示下发失败
                        sbPullUp.push(users[i].uissi + GetTextByName("PullUp_sendfail", useprameters.languagedata) + ";");
                        writeLog("system", users[i].uissi + GetTextByName("PullUp_sendfail", useprameters.languagedata));
                    } else {
                        //下发成功
                    }
                } catch (ex) {
                    sbPullUp.push(users[i].uissi + GetTextByName("PullUp_error_sendfail", useprameters.languagedata));
