...
MzTreeView.addProperty("defaultTarget", "");
...
MzTreeView.prototype.render = function()
{
  ...
  loadImg(MzTreeView.icons.mouseMenu);
  loadImg(MzTreeView.icons.terminal);
  me.firstNode = null;
  loadCssFile(this.iconPath +"mztreeview.css", "MzTreeView_CSS");

  this.initialize(); var str="no data", i, root=this.rootNode;
  if (root.hasChild) {
      var a = [], c = root.childNodes; me.firstNode = c[0];
      for (i = 0; i < c.length; i++) a[i] = c[i].render(i == c.length - 1);
      str = a.join("");
  }
...
MzTreeView.prototype.attachEventArrow = function()
{
  var a = document.getElementById("MTV_root_"+ this.index); if(!a) return;
    //将attachEvent改为attachEventListen--------------------xzj--2018/7/19----------------------------
  var me = this; a.addEventListener("keydown", function (e)
  {
...
MzTreeNode.prototype.render = function (last) {
    ...
    if ($.useCheckbox) {
        if (p.getAttribute("isCheckbox") == "true") {
