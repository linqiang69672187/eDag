<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ResultListSC.aspx.cs" Inherits="Web.lqnew.opePages.GPSContral.ResultListSC" %>
...
<head>
    ...
    <script type="text/javascript">
        ...
        var Log_GPS_ParamEdit_Success = window.parent.GetTextByName("Log_GPS_ParamEdit_Success", window.parent.useprameters.languagedata);
        var Lang_GPSDest = window.parent.GetTextByName("Lang_GPSDest", window.parent.useprameters.languagedata);
        var Lang_GPSCritical = window.parent.GetTextByName("Lang_GPSCritical", window.parent.useprameters.languagedata);
        var thisSendTDGFlashTime = parseFloat(window.parent.sendTDGFlashTime) / 1000;
        function CancalDo() {
            if (!confirm(LangSureTOCancelDoing+"?")) {
                return;
            }
            window.parent.clear_gpscontralSC_p_run();
            document.getElementById("resultContent").innerHTML += "<br>"+LangYouHaveCancel;
            document.getElementById("qxzx").style.display = "none";
        }
        function toHTML(totalcount, havedocount, mbdz, mbzq, cjarry, sbarry) {
            var cjcount = cjarry.length;
            var sbcount = sbarry.length;
            var strtitle = Log_GPS_ParamEdit_Success + "-" + Lang_GPSDest + "(" + mbzq + ")," + Lang_GPSCritical + "(" + mbdz + ")";
            var wcgs = "";
            var otherstr = "";
            ...
            var wzarraycount = totalcount - parseFloat(cjcount) - parseFloat(sbcount);//未知个数=总数量-成功-失败
            ...
            otherstr += LangRemainDoingTime + ":" + (parseFloat(totalcount) - parseFloat(havedocount)) * thisSendTDGFlashTime + "秒";
            otherstr += "<br>" + LangHaveDoingSUCEESSCount + ":" + cjcount;
            otherstr += "   " + LangHaveDoingSUCEESSPercent + ":" + changeTwoDecimal((parseFloat(parseFloat(cjcount) / parseFloat(totalcount))) * 100) + "%";

            otherstr += "<br>" + LangHaveDoingFailedCount + ":<span id='span_sb'>" + sbcount + "</span>";
            otherstr += "   " + LangHaveDoingFailedPercent + ":" + changeTwoDecimal((parseFloat(parseFloat(sbcount) / parseFloat(totalcount))) * 100) + "%";

            otherstr += "<br>" + LangUnKnownCount + ":" + wzarraycount;
        ...
        window.onload = function () {
            ...
            document.onmousedown = function () {
                var div1 = window.parent.document.getElementById('GPSContral/ResultListSC');
        ...
        function closethispage() {
            ...
            window.parent.lq_closeANDremovediv('GPSContral/ResultListSC', 'bgDiv');
        }
