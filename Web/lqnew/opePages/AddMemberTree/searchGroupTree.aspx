<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="searchGroupTree.aspx.cs" Inherits="Web.lqnew.opePages.AddMemberTree.searchGroupTree" %>
                                ...
                                onClick: function (node) {
                                    if (node.entityId == null || node.entityId == undefined || node.entityId == "") {
                                        if (node.type == "zhishu") {
                                            ...
                                            window.parent.parent.selectData(node.id, "-1", "group", "", node1.text);
                                        } else { window.parent.parent.selectData(node.id, "-1", "group", "", node.text); }
                                    } else {
                                        window.parent.parent.selectData(node.entityId, node.text, "group", "", node.text);
                                    }
                                },
