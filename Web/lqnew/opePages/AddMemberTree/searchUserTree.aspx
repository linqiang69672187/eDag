<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="searchUserTree.aspx.cs" Inherits="Web.lqnew.opePages.AddMemberTree.searchUserTree" %>
    ...
    <form id="form1" runat="server" style="height: 380px;">
        ...
        <script type="text/javascript">
            ...
            $.ajax({
                ...
                data: "loginUserId=" + Cookies.get("loginUserId"),
                                ...
                                onClick: function (node) {
                                    if (node.entityId == null || node.entityId == undefined || node.entityId == "") {
                                        if (node.type == "zhishu") {
                                            ...
                                            window.parent.parent.selectData(node.id, "-1", "user", "", node1.text);
                                        } else { window.parent.parent.selectData(node.id, "zhishu", "user", "", node.text); }
                                    } else {
                                        window.parent.parent.selectData(node.entityId, node.text, "user", "", node.text);
                                    }
                                },
                                onBeforeExpand: function (node, param) {
                                    $('#entityAndTypeTree').tree('options').url = "../../../Handlers/resPermissions/getSubEntityAndUsertypeByEntityIdRecursive.ashx?loginuserEntityId=" + Cookies.get["id"] + "&nodeId=" + node.id;
                                }
                            });
                        }
                    } else {
