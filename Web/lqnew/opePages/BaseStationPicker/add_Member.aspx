<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="add_Member.aspx.cs" Inherits="Web.lqnew.opePages.BaseStationPicker.add_Member" %>
    ...
    <script type="text/javascript">
        ...
        function insertr(data, action) {
            for (data_i = 0; data_i < data.length; data_i++) {
                ...
                var switchID = data[data_i]["switchID"];//xzj--20181217--添加交换
                if (action == "add") {
                    createtr(switchID,ISSI, name);
                }
                else {
                    removetr(switchID,ISSI);
                }

            }

        }
        function createtr(switchID,issi, name) {
            var tr = document.getElementById(switchID+","+issi);
            if (tr) {
                return;
            }
            $("#GridView1").prepend("<tr id=" + switchID + "," + issi + " onmouseover='changeTgBg(this,&#39;font&#39;)' onmouseout='overchangeTgBg(this,&#39;font&#39;)' style='background-color:White;height:22px;'><td align='center'  style='white-space:nowrap;cursor:hand;padding:4px;'>" + switchID + "</td><td align='center'  style='white-space:nowrap;cursor:hand;padding:4px;'>" + issi + "</td><td align='center'  style='white-space:nowrap;cursor:hand;padding:4px;'>" + name + "</td><td align='center' ><img src='../../images/010.gif' onmouseout=javascript:dragEnable='True' onmouseover=javascript:dragEnable='False' onclick=removetr(" +switchID+","+ issi + ") style='cursor:hand;' /></td></tr>");
            $(window.frames["smstree"].frames["smsusetree"].document).find("#div" +switchID+","+ issi).append("<img style='margin-left: 3px' id=img" +switchID+","+ issi + " src='../../images/1311402216_yes.png' />");


        }
        function removetr(switchID, id) {    //xzj--20181217--添加交换        
            var selectToDeleteTr = document.getElementById(switchID + "," + id);
            selectToDeleteTr.parentNode.removeChild(selectToDeleteTr);
            $(window.frames["smstree"].frames["smsusetree"].document).find("#img" +switchID+","+ id).remove();
        ...
        function sendok() {
            ...
            if (tbl.rows.length > 1) {
                ...
                for (var i = 0; i < tbl.rows.length; i++) {
                    retrunissis = { name: tbl.rows[i].cells[2].innerHTML, issi: tbl.rows[i].cells[1].innerHTML, switchID: tbl.rows[i].cells[0].innerHTML };//xzj--20181217--添加交换
                    //retrunissis = tbl.rows[i].cells[0].innerHTML + ";" + tbl.rows[i].cells[1].innerHTML;
                }
            }
            ...
            if (sendMsgwindo) {
                ...
                window.parent.lq_closeANDremovediv('BaseStationPicker/add_Member', 'bgDiv');
            }
        }
        var dragEnable = 'True';
        function dragdiv() {
            var div1 = window.parent.document.getElementById('BaseStationPicker/add_Member');
        ...
        window.onload = function () {
            ...
            if (Request.QueryString("issi") == '')
                return;
            if (/[^\d]/.test(Request.QueryString("issi"))) {

                return;
            }

            param.push({ "ISSI": Request.QueryString("issi"), 'name': decodeURI(Request.QueryString("name")), 'switchID': Request.QueryString("switchID") });//xzj--20181217--添加交换
    ...
    <form id="form1" runat="server">
        ...
        <table class="style1" cellpadding="0" cellspacing="0">
            <tr style="height: 5px;">
                ...
                <td class="bg1">
                    <div id="divClose" onmouseover="Divover('on')" onclick="window.parent.lq_closeANDremovediv('BaseStationPicker/add_Member','bgDiv')" onmouseout="Divover('out')"></div>
                </td>
                <td class="style2">
                    ...
                    <table cellpadding="0" cellspacing="0" class="style3">
                                                                                ...
                                                                                <div class="divgrd" id="tbtbz">
                                                                                    <table cellspacing="1" cellpadding="0" style="background-color: #C0DE98; border-color: White; border-width: 0px; border-style: Ridge; width: 99%;">
                                                                                        <tr class="gridheadcss" style="font-weight: bold; font-size: 12px;">
                                                                                            <th scope="col" align="center" style="white-space: nowrap; width: 80px;" id="switchHeadTh">
                                                                                                <%--交换--%>
                                                                                            </th>
                                                                                            <th scope="col" align="center" style="white-space: nowrap; width: 80px;" id="BaseStationIdentification">
                                                                                                <%--基站标识--%>
                                                                                            </th>
                                                                                            <th scope="col" align="center" style="white-space: nowrap; width: 160px;" id="StationName">
                                                                                                <%-- 基站名称--%>
                                                                                            </th>
                                                                                            <th scope="col">&nbsp;
                                                                                            </th>
                        ...
                        <tr>
                            <td colspan="3" align="left" height="35px">
                                <img style="margin-top: 3px; cursor: pointer;" src="../../images/edit_entity.png" src="../../images/send_SMS.png" onclick="sendok()" />&nbsp;
                     <img style="margin-top: 3px; cursor: pointer;" src="../../images/add_cancel.png" onclick="window.parent.lq_closeANDremovediv('BaseStationPicker/add_Member','bgDiv')" /></td>
                        </tr>
                    </table>
...
<script>
    ...
    window.document.getElementById("BaseStationIdentification").innerHTML = window.parent.parent.GetTextByName("BaseStationIdentification", window.parent.parent.useprameters.languagedata);
    window.document.getElementById("switchHeadTh").innerHTML = window.parent.parent.GetTextByName("switchID", window.parent.parent.useprameters.languagedata);//xzj--20181217--添加交换
    window.document.getElementById("StationName").innerHTML = window.parent.parent.GetTextByName("StationName", window.parent.parent.useprameters.languagedata);
