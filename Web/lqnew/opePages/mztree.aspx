<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="mztree.aspx.cs" Inherits="Web.lqnew.opePages.mztree" %>
...
<body style="width:1000px">
    <script type="text/javascript">
        window.onload = function () {
            ...
            a.expand(1);
            if (a.getNodeById("entityroot") != null) {
                a.expand("entityroot");
            }
        ...
        <script type="text/javascript" >
           ...
           "' target='" + target + "' id='" + $.index + "_link_" + id + "' class='MzTreeView'";
                ...
                MzTreeNode.prototype.check = function (checked) {
                    ...
                    var str = 0;
                    str++;
                    if (me.id.replace(/[^a-z,A-Z]/ig, "") == "entity") {
                ...
                MzTreeNode.prototype.upCheck = function () {
                    ...
                    if (node_parentnode && node_parentnode.hasCheckbox) {
                ...
                MzTreeNode.prototype.getselectedentityids = function () {
                    ...
                    var me = this;
                        ...
                        for (var i = 0; i < me_childNodes_length; i++) {
                            ...
                            if (nodetype_getentity == "zhishuuser" && child_Node.checked) {
                                selectedentityids.alone += child_Node.id.replace(/[^0-9]/ig, "") + ",";
                            }
                            else if (nodetype_getentity == "entity" && child_Node.checked) {
                                selectedentityids.cycle += child_Node.id.replace(/[^0-9]/ig, "") + ",";
                            }
                            else if (nodetype_getentity == "entity" && child_Node.checkStatus == 2) {
                ...
                function CheckChildBox(n, mztreeobj) {
                    ...
                    var SelectedEntity = "";
                    //本调度台下单位全选
                    var dispatchEntity_checkStatus = mztreeobj.getNodeById("entity" + dispatchentityid).checkStatus;
                    if (dispatchEntity_checkStatus == 1) {
                        SelectedEntity = "";
                    }
                        //本调度台下单位全没选
                        ...
                        //调度台根级单位部分选中
                    else if (dispatchEntity_checkStatus == 2) {
                        if (check_num >= 1 && up_num >= 1) {
                            var ids = mztreeobj.getselectedentityids("entity" + dispatchentityid);
                            //alert("ids=" + ids);
                            if (ids!=null) {
                                SelectedEntity = ids.alone+"/"+ids.cycle;
                            }
                            else {
                ...
                function displaypolicelists(n) {
                    window.parent.parent.parent.document.getElementById("policelistsdiv").style.display = "block";
