...
{
    public partial class manager_Group : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            GridView1.Columns[0].HeaderText = ResourceManager.GetString("Lang-T-GroupNametext");
            GridView1.Columns[1].HeaderText = ResourceManager.GetString("TypeName");
            ...
            GridView1.Columns[5].HeaderText = ResourceManager.GetString("external_system");
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            switch (e.CommandName)
            {
                    ...
                    {
                        Page.ClientScript.RegisterStartupScript(Page.GetType(), "alert", "<script>window.document.getElementById('Lang_AddNew').src = window.parent.GetTextByName('Lang_AddNew', window.parent.useprameters.languagedata);alert('" + ResourceManager.GetString("Delfailmanager_group") + "');</script>");
                        return;
                    }

                    string GSSI = fungroup.GetGroupinfo_byid(ID).GSSI;
                    string err = "";
                    //检查小组呼叫状态，有终端烧有该小组，有通播组包含有该小组
                    #region  checkstatus
                    ...
                    if (fungroup.checkTBgroup_byGSSI(GSSI) > 0)
                    {
                        err += "\\n        " + strMarker + ResourceManager.GetString("TBGrouphaveGroup");//有通播组包含有该小组
                    }
                   #endregion                    

                    if (err==""){
                        fungroup.DelGroupinfo_byid(ID);
                        Page.ClientScript.RegisterStartupScript(Page.GetType(), "alert", "<script>window.document.getElementById('Lang_AddNew').src = window.parent.GetTextByName('Lang_AddNew', window.parent.useprameters.languagedata);alert('" + ResourceManager.GetString("OperationSuccessful") + "');window.parent.updatecallgroup();window.parent.lq_changeifr('manager_Group');</script>");
                    }
                    else
                    {
